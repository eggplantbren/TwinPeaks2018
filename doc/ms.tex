...
methods. With this example, we empirically investigate the accuracy of TwinPeaks
...
that is intermediate between a Lennard-Jones form and a `polymer' form.
Finally, we present a Bayesian image reconstruction example,
...
An implementation in C++ is available at
...
in terms of $\log(X)$, this measure is
\begin{align}
m_{\rm NS} &\propto \frac{\pi(\xx)}{X(\xx)}.
\end{align}
